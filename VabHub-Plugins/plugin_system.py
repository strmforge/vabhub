...
class AsyncPluginManager:
    ...
    def __init__(self):
        self.plugins: Dict[str, AsyncPluginBase] = {}
            ...
            try:
                ...
                for name, obj in inspect.getmembers(module):
                    if (inspect.isclass(obj) and 
                        issubclass(obj, AsyncPluginBase) and 
                        obj != AsyncPluginBase):
                        ...
                        logger.info(f"加载异步插件: {plugin_instance.plugin_info.name}")
                        
            except Exception as e:
                logger.error(f"加载异步插件 {py_file} 失败: {e}")
    
    async def enable_plugin(self, plugin_name: str) -> bool:
        """启用异步插件"""
        if plugin_name not in self.plugins:
            logger.error(f"异步插件 {plugin_name} 不存在")
            return False
        
        plugin = self.plugins[plugin_name]
        plugin.plugin_info.status = PluginStatus.ENABLED
        logger.info(f"启用异步插件: {plugin_name}")
        return True
    ...
    async def disable_plugin(self, plugin_name: str) -> bool:
        ...
        logger.info(f"禁用异步插件: {plugin_name}")
        return True
    
    async def execute_plugin(self, plugin_name: str, data: Dict[str, Any]) -> Dict[str, Any]:
        """异步执行插件"""
        if plugin_name not in self.plugins:
            return {"success": False, "error": f"异步插件 {plugin_name} 不存在"}
        
        plugin = self.plugins[plugin_name]
        if plugin.plugin_info.status != PluginStatus.ENABLED:
            return {"success": False, "error": f"异步插件 {plugin_name} 未启用"}
        
        try:
            result = await plugin.execute(data)
            return {"success": True, "data": result}
        except Exception as e:
            logger.error(f"执行异步插件 {plugin_name} 失败: {e}")
...
# 全局异步插件管理器实例
async_plugin_manager = AsyncPluginManager()
